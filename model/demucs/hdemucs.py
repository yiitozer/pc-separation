...
from model.modules import SepConv
...
class HEncLayer(nn.Module):
    def __init__(self, chin, chout, kernel_size=8, stride=4, norm_groups=1, empty=False,
        ...
        if sep_conv:
            self.conv = SepConv(conv_type=klass,
                                ch_in=chin,
                                ch_out=chout,
        ...
        if rewrite:
            if sep_conv:
                self.rewrite = SepConv(conv_type=klass,
                                       ch_in=chout,
                                       ch_out=2*chout,
        ...
        self.dconv = None
        if dconv:
            self.dconv = DConv(chout, **dconv_kw, sep_conv=sep_conv)

    def forward(self, x, inject=None, inject_coeff=None):
        """
        ...
        if inject is not None:
            ...
            # Inject coeff should come from median filtering.
            if inject_coeff is not None:
                inject_coeff = inject_coeff.unsqueeze(1).unsqueeze(1)
                y = y * (1 - inject_coeff) + inject * inject_coeff
...
class HDecLayer(nn.Module):
    def __init__(self, chin, chout, last=False, kernel_size=8, stride=4, norm_groups=1, empty=False,
                 ...
                 context_freq=True, rewrite=True, sep_conv=False):
        """
        ...
        if sep_conv:
            self.conv_tr = SepConv(conv_type=klass_tr,
                                   ch_in=chin,
                                   ch_out=chout,
                                   kernel_size=kernel_size,
                                   stride=stride)
        ...
        self.dconv = None
        if dconv:
            self.dconv = DConv(chin, **dconv_kw, sep_conv=sep_conv)
...
class HDemucs(nn.Module):
    ...
    def __init__(self,
        ...
        assert self.time_dec or self.freq_dec, 'At least one decoder is needed.'
        ...
        for index in range(depth):
            ...
            enc = HEncLayer(chin_z, chout_z,
                            dconv=dconv_mode & 1, context=context_enc, sep_conv=sep_conv_enc, **kw)
            if hybrid and freq:
                tenc = HEncLayer(chin, chout, dconv=dconv_mode & 1, sep_conv=sep_conv_enc, context=context_enc,
                                 empty=last_freq, **kwt)
            ...
            if freq_dec:
                dec = HDecLayer(chout_z, chin_z, dconv=dconv_mode & 2,
                                last=index == 0, context=context, **kw_dec, sep_conv=sep_conv_dec)
                if multi:
                    dec = MultiWrap(dec, multi_freqs)
                self.decoder.insert(0, dec)

            # Only for the shared decoders,
            if not freq_dec and index >= (depth - 2):
                dec = HDecLayer(chout_z, chin_z, dconv=dconv_mode & 2,
                                last=index == 0, context=context, **kw_dec, sep_conv=sep_conv_dec)
                self.decoder.insert(0, dec)

            if hybrid and time_dec and freq:
                tdec = HDecLayer(chout, chin, dconv=dconv_mode & 2, empty=last_freq,
                                 last=index == 0, context=context, **kwt, sep_conv=sep_conv_dec)
    ...
    def forward(self, mix):
        ...
        # for weighting the bottleneck layer
        if self._hpss_latent:
            x_mag = z.abs()
            mask_h, mask_p = hpss(x_mag)
            sum_h = mask_h.sum(axis=2).mean(axis=1)
            sum_p = mask_p.sum(axis=2).mean(axis=1)

            inject_coeff = sum_p / (sum_h + sum_p)
        else:
            inject_coeff = None
        ...
        for idx, encode in enumerate(self.encoder):
            ...
            if self.hybrid and idx < len(self.tencoder):
                ...
                if not tenc.empty:
                    if self.time_dec:
                        # save for skip connection, if the time decoder is used
                        saved_t.append(xt)
                else:
                    # tenc contains just the first conv., so that now time and freq.
                    # branches have the same shape and can be merged.
                    inject = xt
            x = encode(x, inject, inject_coeff)
        ...
        for idx in range(self.depth):
            ...
            if idx < len(self.decoder):
                # This is a workaround to handle the decoding stage, if the time decoder
                # is not used. In this case, self.decoder only contains the shared layers.
                # I know that it's not a nice coding style :)
                decode = self.decoder[idx]
                x, pre = decode(x, skip, lengths.pop(-1))
                # `pre` contains the output just before final transposed convolution,
                # which is used when the freq. and time branch separate.
            if self.time_dec:
            # If time decoder is used, than apply the following
                if self.hybrid :
        ...
        if self.freq_dec:
            # If the frequency decoder is not used, x may be forgotten from this point on.
            x = x.view(B, S, -1, Fq, T)
        ...
        if self.hybrid and self.time_dec:
            # If the frequency decoder is not used, x may be forgotten from this point on.
            xt = xt.view(B, S, -1, length)
            xt = xt * stdt[:, None] + meant[:, None]

        if self.hybrid and self.freq_dec and self.time_dec:
            x = xt + x

        elif self.hybrid and self.time_dec and not self.freq_dec:
            return xt

        if self._hpss_output:
            return x, xh, xt
        else:
            return x

    def forward_corr(self, mix):
        x = mix
        length = x.shape[-1]
        ...
        for idx, encode in enumerate(self.encoder):
            ...
            if self.hybrid and idx < len(self.tencoder):
                ...
                tenc = self.tencoder[idx]
                xt = tenc(xt)
                if not tenc.empty and self.time_dec:
        ...
        if self.hybrid:
            ...
            xt = xt * stdt[:, None] + meant[:, None]
            time_output = xt
            freq_output = torch.clone(x)

            x = xt + x
        return x, time_output, freq_output
