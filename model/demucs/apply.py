...
def apply_model_hpss(model, mix, shifts=1, split=True,
                     overlap=0.25, transition_power=1., progress=False, device=None,
                     num_workers=0, pool=None):
    """
    Apply model to a given mixture.
    ...
    if isinstance(model, BagOfModels):
        # Special treatment for bag of model.
        # We explicitely apply multiple times `apply_model_hpss` so that the random shifts
        # are different for each model.
        estimates = 0
        estimates_h = 0
        estimates_p = 0
        ...
        for sub_model, weight in zip(model.models, model.weights):
            ...
            out, out_h, out_p = apply_model_hpss(sub_model, mix, **kwargs)
            sub_model.to(original_model_device)
            for k, inst_weight in enumerate(weight):
                out[:, k, :, :] *= inst_weight
                totals[k] += inst_weight
        ...
        for k in range(estimates.shape[1]):
            estimates[:, k, :, :] /= totals[k]
            estimates_h[:, k, :, :] /= totals[k]
            estimates_p[:, k, :, :] /= totals[k]

        return estimates, estimates_h, estimates_p
    ...
    if shifts:
        ...
        for _ in range(shifts):
            offset = random.randint(0, max_shift)
            shifted = TensorChunk(padded_mix, offset, length + max_shift - offset)
            shifted_out, shifted_out_h, shifted_out_p = apply_model_hpss(model, shifted, **kwargs)
            out += shifted_out[..., max_shift - offset:]
            out_h += shifted_out_h[..., max_shift - offset:]
            out_p += shifted_out_p[..., max_shift - offset:]

        out /= shifts
        out_h /= shifts
        out_p /= shifts
        return out, out_h, out_p
    elif split:
        kwargs['split'] = False
        out = th.zeros(batch, len(model.sources), channels, length, device=mix.device)
        out_h = th.zeros(batch, len(model.sources), channels, length, device=mix.device)
        out_p = th.zeros(batch, len(model.sources), channels, length, device=mix.device)
        ...
        for offset in offsets:
            chunk = TensorChunk(mix, offset, segment)
            future = pool.submit(apply_model_hpss, model, chunk, **kwargs)
            futures.append((future, offset))
            offset += segment
        if progress:
            futures = tqdm.tqdm(futures, unit_scale=scale, ncols=120, unit='seconds')
        for future, offset in futures:
            chunk_out, chung_out_h, chung_out_p = future.result()
            chunk_length = chunk_out.shape[-1]

            out[..., offset:offset + segment] += (weight[:chunk_length] * chunk_out).to(mix.device)
            out_h[..., offset:offset + segment] += (weight[:chunk_length] * chung_out_h).to(mix.device)
            out_p[..., offset:offset + segment] += (weight[:chunk_length] * chung_out_p).to(mix.device)
        ...
        padded_mix = mix.padded(valid_length).to(device)
        with th.no_grad():
            out, out_h, out_p = model(padded_mix)
        return center_trim(out, length), center_trim(out_h, length), center_trim(out_p, length)
