...
def make_filterbanks(n_fft=4096,
                     n_hop=1024,
                     center=False,
                     device=torch.device('cuda')) -> Tuple[nn.Module, nn.Module]:
    window = nn.Parameter(torch.hann_window(n_fft), requires_grad=False).to(device)
